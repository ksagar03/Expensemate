...
const Home = () => {
  ...
  useEffect(() => {
    if (fetchedExp.length > 0) {
      const totalExpenses = fetchedExp.reduce((acc, expense) => acc + expense, 0);
  ...
  sm:max-w-full sm:h-auto sm:ml-0 sm:p-4 sm:text-center bg-[length:200%_200%] animate-gradient"
          >
            <div>
            ...
            Total expenses : {Formater("1234732.21")}
              ...
              key={currentVal}
                ...
                opacityTiming={{ duration: 350, easing: "eas-out" }}
              />
            </div>
          </Card>
          <Graph />
